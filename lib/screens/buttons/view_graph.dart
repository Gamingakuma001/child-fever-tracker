...
class ViewGraphPage extends StatefulWidget {
  const ViewGraphPage({super.key});

  @override
  State<ViewGraphPage> createState() => _ViewGraphPageState();
}
...
class _ViewGraphPageState extends State<ViewGraphPage> {
  ...
  Future<void> _fetchTemperatureData() async {
    ...
    try {
      ...
      final rawData = snapshot.docs.map((doc) {
        final temp = doc['temperature'] as num;
      ...
      for (int i = 0; i < rawData.length; i++) {
        ...
        if (i < rawData.length - 1) {
          ...
          if (nextTs.difference(ts).inMinutes > 30) {
            final baselineTime = ts.add(const Duration(minutes: 15));
            final recoveryTime = nextTs.subtract(const Duration(minutes: 15));

            points.add(FlSpot(baselineTime.millisecondsSinceEpoch.toDouble(), 95.0));
            points.add(FlSpot(recoveryTime.millisecondsSinceEpoch.toDouble(), 95.0));
          }
        }
      }
  ...
  Color getColorForTemp(double temp) {
    ...
    if (temp <= 104) return Colors.yellow.shade700;
    return Colors.red;
  }
  ...
  Widget _buildGraph() {
    ...
    // === Create midnight markers and date labels ===
    ...
    for (final point in _graphPoints) {
      ...
      dayStarts.add(midnight);
    }

    final verticalDaySeparators = dayStarts
        .where((d) => d.isAfter(DateTime.fromMillisecondsSinceEpoch(minX.toInt())) &&
      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
            ...
            // ==== Date labels under vertical lines ====
            for (final d in verticalDaySeparators)
              Positioned(
                left: ((d.millisecondsSinceEpoch.toDouble() - minX) /
                            (maxX - minX) *
                            chartWidth) -
                        25,
                bottom: 5,
